...
from vhm import conversation as conversation_lib
from vhm.constants import (DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN,
                               DEFAULT_IMAGE_TOKEN, IGNORE_INDEX)
from vhm.mm_utils import tokenizer_image_token
from vhm.model import *
from vhm.train.vhm_trainer import VHMTrainer
...
class ModelArguments:
    ...
    mm_vision_select_layer: List[int] = field(default_factory=lambda: [8, 16, 24])   # default to the last layer
...
class LazySupervisedDatasetForRS(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(LazySupervisedDatasetForRS, self).__init__()
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = LazySupervisedDatasetForRS(tokenizer=tokenizer,
...
def train():
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
    trainer = VHMTrainer(model=model,
                             tokenizer=tokenizer,
                             args=training_args,
                             **data_module)
